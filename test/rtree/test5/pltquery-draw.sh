...
# Run this file in bash with ./pltquery-draw.sh -q0.3   i.e.

# get query set size from console:
while getopts q: flag   # you may add extra s.a. flags d:x:a:
do
    case "${flag}" in
        q) qs=${OPTARG};;
    esac
...
gnuplot -persist <<-EOFMarker
	...
	plot "~/eclipse-workspace/test-build/plt/pltquery$qs" using 1:2 w l title "AQAR=$qs" ls 2
