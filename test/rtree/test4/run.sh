...
$bindir/test-rtree-Generator 10000 0 > d
awk '{if ($1 == 1) print $0}' < d > $data
awk '{if ($1 == 2) print $0}' < d > $queries
rm -rf d

echo Creating new R-Tree
$bindir/test-rtree-RTreeLoad $data $tree 20 selfjoin

echo Querying R-Tree
$bindir/test-rtree-RTreeQuery $queries $tree selfjoin > res
cat $data $queries > .t

echo Running exhaustive search
$bindir/test-rtree-Exhaustive .t selfjoin > res2
